...
if(isset($_POST['save1'])|| $id != '' ){
    ...
    if(mysqli_num_rows($sele)>0){
        $fff=mysqli_fetch_array($sele);
            $pid=$fff['pid'];
            $updddd=mysqli_query($conn,"UPDATE billing set pid='".$pid."' where id='$id'");
    }       
...
<td class="tb-col"><span>
   ...
   <tfoot><tr>
    ...
    <td class="tb-col-end" style="background:#56b37d!important;color:white">Rs.<?=$fe['net_amount'];?></td></tr><?php if($fe['r_amount']!=0 && $fe['credit']!=0){?><tr>
        <td colspan="4"></td>
    ...
    <?php }?></tfoot></table></div></div></div></div></div></div></div></div></div>                     
<?php            
        }
    }
?>
